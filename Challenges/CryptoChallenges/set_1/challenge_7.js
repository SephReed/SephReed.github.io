var encryptedFile = "CRIwqt4+szDbqkNY+I0qbDe3LQz0wiw0SuxBQtAM5TDdMbjCMD/venUDW9BL "
...
var decryptMe = encryptedFile.replace(/ /g, '');
decryptMe = CT.hexToString(CT.base64ToHex(decryptMe));
...
var keyText = "YELLOW SUBMARINE";
var key = [];
for(var c in keyText) 
	key[c] = keyText.charCodeAt(c);
...
// 	var out = '';
// 	var cbcXor;
// 	if(cbcMode === true && backToFront !== true) {
// 		cbcXor = '';
// 		for (var i = 0; i < 16; i++)
// 			cbcXor += String.fromCharCode(Math.floor(Math.random()* 0xff));
...
// 	if(backToFront !== true) {
// 		for(var i = 0; i < decryptMe.length; i += BLOCK_SIZE) {
// 			var block = new Array(BLOCK_SIZE);

// 			var chunk = decryptMe.substr(i, BLOCK_SIZE);
// 			for (var c in chunk) 
...
// 			if(cbcMode === true) {
// 				addMe = CT.xorString(addMe, cbcXor);
// 				cbcXor = addMe;
// 			}
...
// 		}
// 	}
// 	else alert("fix CT.chainBlocks");

// 	return out;
// }


console.log(CT.ECB(decryptMe, AES_Decrypt, key));
...
// 	for (var b in block)
// 		out += String.fromCharCode(block[b])
// }
