...
// static void
// function key_add(rijn_block_t *out, rijn_block_t *in, rijn_block_t *roundkey, int numrows)
// {
...
// }
function key_add(out, in, roundkey, numrows) {
    var flat_out = UInt8(out);
    var flat_in = (unsigned char *) in;
    var flat_rk = (unsigned char *) roundkey;
    var numbytes = numrows_to_blocksize(numrows);
...
function shift_column_routine(block, numrows, inverse) {
    // var shifts[2][3][4] = {
    //     {
    //         { 0, 1, 2, 3 }, /* shifts for 128 bit block */
    //         { 0, 1, 2, 3 }, /* shifts for 192 bit block */
...
function shift_column(rijn_block_t *block, rijn_size_t numrows) {
    shift_column_routine(block, numrows, 0);
}

function shift_column_inverse(rijn_block_t *block, rijn_size_t numrows) {
    shift_column_routine(block, numrows, 1);
}

// function substitute(rijn_block_t *block, int *box, int numrows) {
...
// }
function substitute(block, box, numrows) {
    var flat_block = block;
    var numbytes = numrows_to_blocksize(numrows);
...
function mix_row(block, numrows) {
    var temp;
...
// static void
function mix_row_inverse(block, numrows) {
    var temp;
...
function rijn_param_init(param, blocksize, keysize)
{
...
// function rijn_sched_key(rijn_keysched_t *sched, rijn_key_t *key, const rijn_param_t *param)
function rijn_sched_key(sched, key, const rijn_param_t *param)
{
